#pragma once
#include "Il2Cpp/Il2Cpp.h"

class doFetchConfigd29
{
public: 

	static Il2CppClass *StaticClass() {
		return (Il2CppClass *)(Il2Cpp::GetClass("Assembly-CSharp.dll", "", "<doFetchConfig>d__29"));
	}

	template <typename T = int32_t> T& 1__state() {
		return *(T*)((uintptr_t)this + 0x0);
	}
	template <typename T = uintptr_t> T& t__builder() {
		return *(T*)((uintptr_t)this + 0x8);
	}
	template <typename T = void*> T& u__1() {
		return *(T*)((uintptr_t)this + 0x20);
	}
	template <typename T = void*> T& u__2() {
		return *(T*)((uintptr_t)this + 0x28);
	}
	template <typename T = uintptr_t> T& u__3() {
		return *(T*)((uintptr_t)this + 0x30);
	}

	template <typename T = void> T MoveNext() {
		return ((T (*)(doFetchConfigd29*))(Il2CppBase() + 0x16F2054))(this);
	}
	template <typename T = void> T SetStateMachine(uintptr_t stateMachine) {
		return ((T (*)(doFetchConfigd29*, uintptr_t))(Il2CppBase() + 0x16F205C))(this, stateMachine);
	}

};

}
